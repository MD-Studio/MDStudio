...
class LIEWorkflow(LieApplicationSession):
    ...
    def task(self, task_data, callback=None, *args, **kwargs):
        ...
        # Call the service
        deferred = yield self.call(
            task_data['uri'], *inp.values(), session=session.dict(), **conf)
    ...
    def onRun(self, details):
        ...
        lig_cids = {
            'cid001': None, 'cid002': None, 'cid003': None, 'cid004': None,
            'cid005': None}
        self.logger.info('Dock {0} structures'.format(len(lig_cids)))
        for ligand in lig_cids:
            lig = yield self.task(
                u'liestudio.structures.get_structure', ligand)
            lig_cids[ligand] = lig
        ...
        for ligid, ligand in project['ligands'].items():
            docked[ligid] = self.task(
                u'liestudio.plants_docking.run_docking',
                project['protein']['result'],
                ligand['result'],
                workdir='/tmp',
        ...
        for ligid, ligand in project['ligands'].items():
            atb[ligid] = self.task(
                u'liestudio.atb.structure_query', ligand['result'],
        ...
        for atbq in atb:
            ...
            for query_result in f['result']:
                if query_result.get('is_identical', False):
                    self.logger.info(
                        'Exact ATB match found for {0}, molid: {1}'.format(
                            atbq, query_result.get('molid')))
                    topol = yield self.task(
                        u'liestudio.atb.get_topology',
                        query_result.get('molid'))
...
if __name__ == '__main__':
    ...
    certpath = os.path.join(
        os.path.dirname(__file__), '../data/crossbar/server_cert.pem')
    cert = crypto.load_certificate(
        crypto.FILETYPE_PEM, open(certpath, 'r').read())

    # tell Twisted to use just the one certificate we loaded to
    # verify connections
    ...
    runner = ApplicationRunner(
        ...
        u"liestudio",
        extra={
            'authid': u'lieadmin', 'password': u'liepw@#',
            'authmethod': u'ticket'},
        # ssl=options
    )
