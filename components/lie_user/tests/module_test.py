...
class UserDatabaseTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        * Write MongoDB log to local mongodb.log file
        ...
        user = client['users']
        user.insert_many([
          {'username':'test1', 'email':'test1@email.com', 'password':hash_password('test1'), 'role':'default', 'uid':0, 'session_id':None},
          {'username':'test2', 'email':'test2@email.com', 'password':hash_password('test2'), 'role':'default', 'uid':1, 'session_id':None},
