...
class LieWampTaskMetadataTest(unittest.TestCase):
    ...
    _settings_json = os.path.join(os.path.dirname(_currpath), 'files')
    ...
    def test_task_default_fromjson(self):
        ...
        jsonmeta = json.load(open(os.path.join(self._settings_json, 'default_task_metadata.json')))
        metadata = WAMPTaskMetaData(metadata=jsonmeta)
    
    def test_task_prefilled_fromjson(self):
        
        jsonmeta = json.load(open(os.path.join(self._settings_json, 'prefilled_task_metadata.json')))
        metadata = WAMPTaskMetaData()
