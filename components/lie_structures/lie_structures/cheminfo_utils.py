...
logging = Logger()
packages = {'pybel': None, 'indy': None, 'rdk': None, 'cdk': None, 'webel': None, 'opsin': None, 'jchem': None}

for package in packages:
    
    try:
        packages[package] = importlib.import_module("lie_structures.cinfony.{0}".format(package))
    except:
...
def mol_read(mol, mol_format=None, from_file=False, toolkit='pybel', fallback='webel', default_mol_name='ligand'):
    ...
    if not mol_format in toolkit_driver.informats:
...
def mol_attributes(molobject):
    ...
    for attr in ('formula', 'molwt', 'title', 'charge', 'dim', 'energy', 'exactmass'):
...
def mol_addh(molobject, polaronly=False, correctForPH=False, pH=7.4):
    ...
    if molobject.toolkit == 'pybel':
        logging.info('Add hydrogens. Toolkit: {0}, only polar: {1}, correct pH: {2}, pH: {3}'.format(molobject.toolkit, polaronly, correctForPH, pH))
...
def mol_removeh(molobject):
    ...
    logging.info('Remove hydrogen atoms from structure: {0}'.format(molobject.title))
    molobject.removeh()
...
def mol_make3D(molobject, forcefield='mmff94', localopt=True, steps=50):
    ...
    if molobject.toolkit == 'cdk':
        logging.error('Conversion to 3D coordinate set not supported by CDK toolkit')
        return None
...
def mol_rotate(molobject, vector=[0,0,0,0]):
    ...
    #calculates a rotation matrix around the axes specified, by angle specified
    matrix.RotAboutAxisByAngle(toolkit_driver.ob.vector3(x,y,z), angle) 
...
def mol_combine_rotations(molobject, rotations=[]):
    ...
    toolkit_driver = packages.get(molobject.toolkit)
    rotated_file = toolkit_driver.Outputfile(molobject.mol_format, "multipleSD.mol2")
