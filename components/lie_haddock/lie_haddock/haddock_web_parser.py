...
def json_to_schema(input_json):
    """
    Create simple JSON schema file based on a JSON data construct
    """
    ...
    def _walk_dict(d, m):
        for k,v in d.items():
            if type(v) == dict:
                m[k] = {"description": "", "type": "object", "properties": {}}
                _walk_dict(v, m[k]['properties'])
            elif type(v) in (str, unicode):
                m[k] = {"description": "", "type": "string", "default":v}
            elif type(v) == bool:
                m[k] = {"description": "", "type": "boolean", "default":v}
            elif type(v) == int:
                m[k] = {"description": "", "type": "integer", "default":v}
            elif type(v) == float:
                m[k] = {"description": "", "type": "number", "default":v}
            elif type(v) == list:
                m[k] = {"description": "", "type": "array", "default":v}
            else:
                raise Exception("Unknown type: {0}, {1}".format(k, v))
        
    _walk_dict(input_json, schema['properties'])
    return schema
...
class HaddockParamWeb(object):
        ...
        try:
            with open(path, 'w') as output:
                if indent and sort_keys:
                    json.dump(self.data, output, indent=4, sort_keys=True)
                elif indent:
    ...
    def change_value(self, key, new_val):
        ...
        try:
            result = list(self._change_value(key, new_val, self.data))
            if result:
                self.data = result[0]
            else:
    ...
    def get_value(self, key):
        dic = self.data
        value = list(self._get_value(key, dic))
        if value:
            if len(value) > 1:
                return value
            else:
