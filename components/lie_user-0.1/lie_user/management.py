...
class UserManager(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.user:
            self._session_key = self.user.get('session_id', None)

    def set_session_key(self):
        """
        Create a session ID based on the current time and
        a random integer. This session ID is valid for the
        ...
        self._session_key = '{0}{1}'.format(epoc, rand)

        users = db['users']
        users.update_one({'uid': self.user['uid']}, {'$set':{'session_id': self._session_key}})

        logging.debug('Open session: {0} for user {1}'.format(self._session_key, self.user['uid']))
    ...
    def validate_user_login(self, username, password):
        ...
        logging.info('{status} login attempt for user: {user}, password: {pw}',
            status='Correct' if check else 'Incorrect', user=username, pw=password)
