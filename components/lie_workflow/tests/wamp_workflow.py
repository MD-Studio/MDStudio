...
class LIEWorkflow(LieApplicationSession):
    ...
    def onRun(self, details):
        ...
        t5 = wf.add_task('ACPYPE', task_type='WampTask', uri='liestudio.amber.acpype')
        wf.input(t5, tmp_dir=workdir)
        ...
        wf.connect_task(t1, t6)
        wf.connect_task(t4, t6, data_mapping={'mol':'structure'})
        
        # Run PLANTS on ligand and protein (in parallel to acpype)
        t7 = wf.add_task('Plants docking', task_type='WampTask', uri='liestudio.plants_docking.run_docking')
        wf.input(t7, workdir=workdir, bindingsite_center=[7.79934,9.49666,3.39229])
